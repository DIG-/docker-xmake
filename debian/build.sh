...
TAG_PREFIX="diguser/xmake-debian"
TAGS=()
XMAKE_VERSION=(${XMAKE//./ })
TAGS+=("-t $TAG_PREFIX:${DEBIAN}-${XMAKE_VERSION[0]}")
TAGS+=("-t $TAG_PREFIX:${DEBIAN}-${XMAKE_VERSION[0]}.${XMAKE_VERSION[1]}")
TAGS+=("-t $TAG_PREFIX:${DEBIAN}-${XMAKE_VERSION[0]}.${XMAKE_VERSION[1]}.${XMAKE_VERSION[2]}")
if [ $XMAKE_LATEST -eq 1 ]; then
    TAGS+=("-t $TAG_PREFIX:${DEBIAN}-latest")
    if [ "$DEBIAN" == "latest" ]; then
